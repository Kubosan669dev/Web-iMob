  ...
  if (!page) {
    return (
      <section className="py-32">
        <Container>
          <p className="text-center text-ink-soft">Không tìm thấy nội dung.</p>
        </Container>
      </section>
    );
  }
    ...
    <section className="relative overflow-hidden py-28 lg:py-32">
      ...
      <Container className="relative max-w-3xl">
        ...
        <header className="mb-12 border-b border-line pb-8">
          ...
          <p className="mt-1 text-sm font-medium uppercase tracking-[0.2em] text-ink-faint">
            {page.subtitle}
          </p>
          <p className="mt-4 text-xs text-ink-faint">{page.effective}</p>
        </header>
        ...
        <div className="space-y-10">
          {page.sections.map((sec, i) => (
            <div key={sec.heading}>
              <h2 className="mb-3 flex items-baseline gap-3 text-lg font-bold text-ink">
              ...
              {sec.paragraphs &&
                sec.paragraphs.map((p) => (
                  <p key={p} className="mb-3 text-sm leading-relaxed text-ink-soft">
                    {p}
                  </p>
                ))}

              {sec.note && (
                <p className="mt-4 rounded-xl border border-line bg-panel px-4 py-3 text-xs leading-relaxed text-ink-soft">
                  {sec.note}
                </p>
              )}
            </div>
        ...
        <div className="mt-14 border-t border-line pt-8">
          <Link
            to="/#home"
            className="inline-flex items-center gap-1.5 text-sm font-semibold text-brand transition-colors hover:text-ink"
          >
